...
def _detect_chord_progressions(chord_sequence: List[str],
    ...
    Returns list of (progression_string, count) sorted by frequency descending.
    """
    if len(chord_sequence) < min_length:
        return []
    
    progression_counts = collections.Counter()
    ...
    return progression_counts.most_common(top_n)
...
def _get_monophonic_melodies(
    ...
    """
    Find monophonic melodies grouped by (channel, patch).
    """
    melodies = []
    song_beats = max_tick / ticks_per_beat if ticks_per_beat > 0 else 1
    ...
    for (ch, patch), notes in patch_notes.items():
        ...
        melody_beats = (melody_end - melody_start) / ticks_per_beat if ticks_per_beat > 0 else 0
...
def _calc_avg_polyphony(all_notes: List[Tuple[int, int, int, int]], 
    ...
    """O(n log n + samples) instead of O(n * samples)."""
    if not all_notes or max_tick <= 0:
        return 0.0
    ...
    for sample_tick in range(0, max_tick, sample_interval):
...
def analyze_midi(filepath: str, top_n_instruments: int = 5,
    ...
    Notes:
        ...
        - Chord detection uses 1-beat windows with template matching against 16 quality types.
    ...
    for ch, notes in channel_notes.items():
        sorted_ch = sorted(notes, key=lambda x: x[0])
        element_counts["arpeggio"] += _count_arpeggios(sorted_ch, ticks_per_beat)
        element_counts["trill"] += _count_trills(sorted_ch, ticks_per_beat)
        element_counts["grace_note"] += _count_grace_notes(sorted_ch, ticks_per_beat)
        element_counts["glissando"] += _count_glissando(sorted_ch, ticks_per_beat)
        element_counts["tremolo"] += _count_tremolo(sorted_ch, ticks_per_beat)
        element_counts["ostinato"] += _count_ostinato(sorted_ch, ticks_per_beat)
        element_counts["syncopation"] += _count_syncopation(sorted_ch, ticks_per_beat, time_sig_num)
        element_counts["run"] += _count_runs(sorted_ch, ticks_per_beat)
        element_counts["mordent"] += _count_mordents(sorted_ch, ticks_per_beat)
    ...
    # Build drum groups summary (all groups used, sorted by group ID)
    drum_groups_summary = []
    for note in drum_hits:
        group_id, group_name, drum_name = _get_drum_group(note)
        if group_id not in [g["group_id"] for g in drum_groups_summary]:
            # Count total hits in this group
            group_hit_count = sum(count for n, count in drum_hits.items() 
                                  if _get_drum_group(n)[0] == group_id)
            # Get all drum names in this group that were used
            used_drums_in_group = [_get_drum_group(n)[2].lower() for n in drum_hits 
                                   if _get_drum_group(n)[0] == group_id]
            drum_groups_summary.append({
                "group_id": group_id,
                "group_name": group_name.lower(),
                "total_hits_desc": _hit_count_to_words(group_hit_count).lower(),
                "drums_used": used_drums_in_group
            })
    
    # Sort by group_id
    drum_groups_summary.sort(key=lambda x: x["group_id"])
    ...
    drum_groups_numeric = [{"group_id": g["group_id"],
                            "total_hits": sum(count for n, count in drum_hits.items() 
                                              if _get_drum_group(n)[0] == g["group_id"]),
                            "drum_note_ids": [n for n in drum_hits 
                                               if _get_drum_group(n)[0] == g["group_id"]]}
